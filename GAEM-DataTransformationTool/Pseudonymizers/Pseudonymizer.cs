...
namespace GAEM.DataTransformationTool.Pseudonymizers
{
    public class Pseudonymizer
    {
        ...
        {
            string inputWithSalt = input + GlobalSettings.GeneralSettings.PseudonymizationSalt;
        ...
        public string PseudonymizeFilePath(string filePath)
        {
            ...
            filePath = filePath.Replace(GlobalSettings.GeneralSettings.PathPrefix, "");

            // hash all folder names (except last, that is the filename)
            string[] pathElements = filePath.Split(new string[] { @"\", "/" }, StringSplitOptions.None); // original '\\' char
        ...
        {
            directoryPath = directoryPath.Replace(GlobalSettings.GeneralSettings.PathPrefix, "");

            string[] pathElements = directoryPath.Split(new string[] { @"\", "/" }, StringSplitOptions.None); // original '\\' char
            for (int i = 0; i < pathElements.Length - 1; i++)
            {
                pathElements[i] = PseudonymizeString(pathElements[i]);
            }
